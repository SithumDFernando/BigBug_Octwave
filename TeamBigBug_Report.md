...
## 1. Approach
Our approach centers on domain-driven feature engineering and optimized boosting algorithms, avoiding synthetic resampling techniques (like SMOTE). We handled the extreme class imbalance (1.51% fraud) by applying algorithmic class weights to penalize minority class misclassification. After engineering 16 new features, we systematically evaluated 9 classification algorithms. We utilized Bayesian Optimization (Optuna) for hyperparameter tuning (20 trials per model), validating results via 5-Fold Stratified Cross-Validation.

## 2. Final Model
Our final model is an **Optuna-Weighted Soft Voting Ensemble** that blends the predicted probabilities of our top 5 performing gradient boosting algorithms:
- CatBoost (weight: 0.268)
- LightGBM (weight: 0.235)
- GradientBoosting (weight: 0.235)
- AdaBoost (weight: 0.147)
- XGBoost (weight: 0.115)
...
- **Ensemble Diversity as Insurance:** Blending 5 distinct boosting algorithms offers the best protection against overfitting to the public leaderboard, maximizing stability for the 70% unseen private test data.
